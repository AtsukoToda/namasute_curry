            ...
            <v-col>
                ...
                >
                    ...
                    <template v-slot:[`item.delete`]="{ item }">
                        <v-btn v-if="show" @click="deleteConfirm(item.cartId)" color="error" 
rounded><strong>削除</strong></v-btn>
                    </template>
                </v-data-table>
            </v-col>
        </v-row>
        <v-row class="ma-2">
            <v-col align="center">
                ...
                <h2>ご注文金額合計：{{(Math.floor(priceSum+(priceSum*tax)).toLocaleString('ja-JP'))}}円
(税込)</h2>
                <v-btn v-if="show" color="orange" dark @click="loginCheck()" rounded href="#orderForm">
                  注文に進む</v-btn>
